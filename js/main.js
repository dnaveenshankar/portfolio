...
/* WebMCP: expose safe, read-only portfolio actions to browser agents. */
(function registerWebMCP() {
    ...
    navigator.modelContext.provideContext({
        tools: [
            {
                name: 'view_portfolio_section',
                description: 'Open a named section of Naveen Shankar D\'s portfolio, such as about, experience, education, skills, projects, certifications, or contact.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        section: {
                            type: 'string',
                            enum: ['about', 'experience', 'education', 'skills', 'projects', 'certifications', 'contact']
                        }
                    },
                    required: ['section']
                },
                execute: async ({ section }) => {
                    ...
                    const target = document.querySelector(aliases[section]);
                    if (!target) return { content: [{ type: 'text', text: `Section not found: ${section}` }] };
                    target.scrollIntoView({ behavior: 'smooth', block: 'start' });
                    return { content: [{ type: 'text', text: `Opened the ${section} section.` }] };
                }
            },
            {
                name: 'download_cv',
                description: 'Open Naveen Shankar D\'s current CV PDF from the portfolio.',
                inputSchema: { type: 'object', properties: {} },
                execute: async () => {
                    window.open('/cv.pdf', '_blank', 'noopener');
                    return { content: [{ type: 'text', text: 'Opened the CV PDF.' }] };
                }
            },
            {
                name: 'contact_naveen',
                description: 'Open the portfolio contact page so a visitor can contact Naveen Shankar D.',
                inputSchema: { type: 'object', properties: {} },
                execute: async () => {
                    window.location.href = '/contact-me.html';
                    return { content: [{ type: 'text', text: 'Opened the contact page.' }] };
                }
            }
        ]
    });
