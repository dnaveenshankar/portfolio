(() => {
  ...
  async function pollLive(render) {
    ...
    try {
      ...
      const previous=state.liveStatus; state.liveStatus=data.status;
      (data.messages||[]).forEach(m=>{ state.lastMessageId=Math.max(state.lastMessageId,Number(m.id)||0); if(m.sender!=="visitor" && typeof render === "function") render(m.message,m.sender); });
      if(data.status==="accepted" && previous!=="accepted" && typeof render === "function") render("You're connected with Naveen now. 💬","naveen-status");
      if((data.status==="declined"||data.status==="expired") && previous!==data.status && typeof render === "function") render(data.status==="declined"?"Naveen couldn't accept the connection right now. I can still help you here. 💙":"The live connection request has expired. If you'd like, you can request another connection.","sana-status");
    } catch {}
  }
  ...
  document.addEventListener("DOMContentLoaded",()=>{
    ...
    document.querySelectorAll("[data-sana-messages]").forEach(list=>{
      ...
      if(first){first.textContent="Hi I'm Sana, Naveen's personal assistant. I'm happy to help here. 💙";first.setAttribute("data-sana-default","1");}
      else {const b=document.createElement("div");b.textContent="Hi I'm Sana, Naveen's personal assistant. I'm happy to help here. 💙";b.className="sana-bot-message";b.setAttribute("data-sana-default","1");list.insertBefore(b,list.firstChild);}
    });
  });
})();
