// Shared sidebar nav + toast notifications for the admin panel.
// Include with <script src="/admin-nav.js"></script> and call renderSidebar()
// after the DOM's #sidebarSlot element exists.

const ADMIN_NAV_SECTIONS = [
  { name: "Dashboard", icon: "🧭", href: "/dashboard.html" },
  { name: "Profile", icon: "👤", href: "/profile-edit.html" },
  { name: "Skills", icon: "🛠️", href: "/skills.html" },
  ...
  { name: "Blog", icon: "✍️", href: "/blog.html" },
  { name: "Quotes", icon: "💬", href: "/quotes.html" },
  { name: "Availability", icon: "🗓️", href: "/availability.html" },
...
function renderSidebar() {
  ...
  slot.innerHTML = `
    <aside class="sidebar">
      <div class="sidebar-brand">🧭 <span>Admin</span></div>
      <nav class="sidebar-nav">
        ${ADMIN_NAV_SECTIONS.map((s) => {
          const isActive =
            currentPath === s.href ||
            (s.href.includes("table=") && currentTable && s.href.includes(`table=${currentTable}`));
          return `<a href="${s.href}" class="${isActive ? "active" : ""}"><span class="icon">${s.icon}</span>${s.name}</a>`;
        }).join("")}
      </nav>
...
// Toast notifications — replaces alert()/confirm() styling with a floating,
// auto-dismissing message. showToast(message, type) where type is
